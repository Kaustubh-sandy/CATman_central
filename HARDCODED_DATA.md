...
| Working states, machine states | safety engine, [ML file](backend/src/ml/anomaly_ensemble.py) `STATES`, simulator, frontend | The ETA dataset was regenerated on 24 Sep to use the simulator's own states (it used `RUNNING` / `STOPPING` before). |
...
| ETA dataset | [ml/eta/data/](backend/src/ml/eta/data/) `train/validation/test.csv`, 50 000 rows | Generated by [generate_dataset.js](backend/src/ml/eta/data/generate_dataset.js) with seed 42: the input rows replay the simulator's state machine and sensor targets; the target `eta_minutes` is **a hand-written formula** (base 12 min + state, load-cycle, rpm, fuel, idle, temperature, vibration, scenario and seatbelt effects, + noise σ 2.0, clamped 3–90). A model trained on it learns that formula, not real machine behaviour. |
